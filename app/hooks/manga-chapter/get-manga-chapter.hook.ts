...
const STALE_TIME = 1000 * 60 * 60 * 24 * 3; // 3d
const RETRY_TIME = 1000 * 5; // 5s

export const useMangaChapter = (chapter?: string) => {
	const isValidSlug = Boolean(chapter?.trim?.()?.length) && chapter?.includes?.("/");

	const { retrieveStoredManga, getManga, updateManga } = useSearchStore();
	const { data, isLoading, isRefetching, refetch } = useQuery({
		queryKey: [SEARCH_STORE_KEY, "MANGA", "CHAPTER", chapter],
	...
	async function retrieveManga() {
		try {
			if (!chapter?.trim?.()?.length || chapter === "undefined") {
				throw Error("Slug not found...");
			}

			const [slug, chapter_slug] = chapter.split?.("/") ?? [];
