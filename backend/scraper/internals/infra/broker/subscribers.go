...
func SetupSubscribers(request SubscriberRequest) {
	...
	go create_consumer(events.SEARCH_MANGA_EVENT, request, search.SearchByNameSubscriber, &wg)
	go create_consumer(events.SCRAPE_MANGA_EVENT, request, manga.ScrapeMangaSubscriber, &wg)
	go create_consumer(events.SCRAPE_CHAPTER_EVENT, request, manga.ScrapeChapterSubscriber, &wg)
