...
func SetupPublishers(request PublisherRequest) {
	create_publisher(events.SEARCHED_MANGA_EVENT, request)

	create_publisher(events.SCRAPED_MANGA_EVENT, request)
	create_publisher(events.SCRAPED_CHAPTER_EVENT, request)
}
